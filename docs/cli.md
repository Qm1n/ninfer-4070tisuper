...
<!-- // Fork: common options include native packed I4-G128 and explicit I4-G64 storage. -->
...
| `--prefill-chunk N` | positive text-prefill chunk, in multiples of 64 | `1024` |
| `--max-new N` | requested output-token limit | `128` |
| `--device N` | CUDA device index | `0` |
| `--kv-dtype bf16\|int8\|i4\|i4-g64` | KV-cache storage; `i4` selects G128 | `bf16` |
...
KV-cache type.
<!-- // Fork: I4-G128 halves code traffic and halves scale rows relative to I4-G64. -->
Use `--kv-dtype i4` (I4-G128) for the largest context allocations, `i4-g64` for the prior
four-scale packed route, or `int8` for the existing quantized profile. The prepared prompt must fit
...
<!-- // Fork: byte-exact startup telemetry supports long-context capacity qualification. -->
Set `NINFER_MEMORY_VERBOSE=1` to print `MemorySummary` immediately after load, including sequence
and workspace capacities, KV payload/encoding, device-hot and pinned-host GDN state, Graph
allowance/observed bytes, and free VRAM after weights and after complete startup.
