...
// Fork: without libcurl the local source kinds keep their Linux behavior and remote URLs fail as
// unavailable instead of being silently ignored.
...
std::vector<std::uint8_t> read_path(const Source& source, const Policy& policy) {
    std::error_code ec;
    std::filesystem::path path(source.value);
...
std::vector<std::uint8_t> acquire_bytes(const Source& source, const Policy& policy) {
    ...
    if (source.kind == SourceKind::Url) {
        throw Error(ErrorKind::RemoteUnavailable,
                    "remote media acquisition is unavailable in this build: it was configured "
                    "without libcurl (NINFER_ENABLE_MEDIA=OFF)");
    }
