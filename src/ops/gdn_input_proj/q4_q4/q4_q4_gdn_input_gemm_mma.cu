...
// Fork: Homogeneous Q4/Q4 large-T GDN projection without high-plane loads.
...
RowSplitGroupedMmaJob make_job(const Weight& weight, std::int32_t weight_row_offset,
    ...
    return RowSplitGroupedMmaJob{codes,
                                 nullptr,
                                 scales,
...
void launch_slice(bool full, const Tensor& x, const Weight& qk_weight,
    ...
    if (full) {
        rowsplit_grouped_mma_kernel<Schedule, true, RowSplitGroupedMmaCodec::Q4, 4>
            <<<grid, Schedule::THREADS, 0, stream>>>(static_cast<const __nv_bfloat16*>(x.data), qk,
                                                     value, output_gate, empty, x.ne[0], cols,
                                                     x.ne[0]);
    } else {
        rowsplit_grouped_mma_kernel<Schedule, false, RowSplitGroupedMmaCodec::Q4, 4>
            <<<grid, Schedule::THREADS, 0, stream>>>(static_cast<const __nv_bfloat16*>(x.data), qk,
                                                     value, output_gate, empty, x.ne[0], cols,
                                                     x.ne[0]);
    }
...
void q4_q4_gdn_input_grouped_mma_launch(const Tensor& x, const Weight& qk_weight,
