...
// Fork: Homogeneous Q4/Q4 large-T attention projection without high-plane loads.
...
RowSplitGroupedMmaJob make_job(const Weight& weight, std::int32_t row_begin, std::int32_t row_count,
    ...
    if (row_begin < 0 || row_count <= 0 || row_begin + row_count > weight.n ||
        out.ne[0] != row_count) {
        throw std::invalid_argument("Q4/Q4 attention input grouped MMA row view is invalid");
    }
    ...
    return RowSplitGroupedMmaJob{codes, nullptr, scales, static_cast<__nv_bfloat16*>(out.data),
                                 row_count, out.ne[0], 0, false};
}
...
void launch_pair(bool full, const Tensor& x, RowSplitGroupedMmaJob first,
    ...
    if (full) {
        rowsplit_grouped_mma_kernel<Schedule, true, RowSplitGroupedMmaCodec::Q4, 2>
            <<<grid, Schedule::THREADS, 0, stream>>>(static_cast<const __nv_bfloat16*>(x.data),
                                                     first, second, empty, empty, x.ne[0], cols,
                                                     x.ne[0]);
    } else {
        rowsplit_grouped_mma_kernel<Schedule, false, RowSplitGroupedMmaCodec::Q4, 2>
            <<<grid, Schedule::THREADS, 0, stream>>>(static_cast<const __nv_bfloat16*>(x.data),
                                                     first, second, empty, empty, x.ne[0], cols,
                                                     x.ne[0]);
    }
...
void launch_slice(const Tensor& x, const Weight& query_key_weight, const Weight& gate_value_weight,
    ...
    const std::int32_t tail  = k.ne[0];
    launch_pair<Schedule>(full, x, make_job(query_key_weight, 0, split, q),
                          make_job(query_key_weight, split, tail, k), stream);
    launch_pair<Schedule>(full, x, make_job(gate_value_weight, 0, split, gate),
                          make_job(gate_value_weight, split, tail, v), stream);
}
...
void q4_q4_attn_input_grouped_mma_r16_c64_s3_launch(const Tensor& x,
...
void q4_q4_attn_input_grouped_mma_r32_c64_s4_launch(const Tensor& x,
                                                     const Weight& query_key_weight,
                                                     const Weight& gate_value_weight, Tensor& q,
                                                     Tensor& gate, Tensor& k, Tensor& v,
                                                     cudaStream_t stream) {
